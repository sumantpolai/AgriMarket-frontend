...
function AddProduct({ onClose , getProducts }) {
  const category = ['grain', 'fruit', 'vegetable', 'dairy', 'meat', 'other']
  const [formData, setFormData] = useState({category:'grain'})
  ...
  const formSubmit = async(e)=>{
    formData.price = Number.parseInt(formData.price)
    formData.discount = Number.parseInt(formData.discount)
    e.preventDefault()
    setSendData(true)
    ...
    <>
      ...
      { sendData && <div className='h-full w-full absolute top-0 z-20 md:pt-64 pt-44 font-bold text-[24px] flex items-center text-white bg-black bg-opacity-80 flex-col  '>
        ...
        <form onSubmit={formSubmit} className=' h-full md:w-[70%] w-full flex flex-col gap-8 relative  text-black '>
          ...
          <h1 className='md:text-[25px] text-[18px] font-bold text-green-500 text-center'>Add your Product</h1>
          <div className='flex flex-col gap-3'>
            ...
            <div className='flex flex-col gap-2'>
             ...
             <input required type="text" name='quantity' id='quantity' value={formData.quantity || ""} onChange={handleForm}  placeholder='ex: 1kg , 130kg , 5l , 10l' className=' border-2 border-black  px-3 py-1 md:py-2 rounded-full outline-none' />
            </div>
            <div className='flex flex-col gap-2'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <textarea required name='description' id='description' value={formData.description || ""} onChange={handleForm}  rows={5}   placeholder='ex: Its a kashmir apple.' className='border-2 border-black rounded-md px-3 py-1 outline-none'></textarea>
            </div>
          </div>
